...
import use_case.demo3.BuildTopRecipesFromPreferencesInteractor;
import use_case.demo3.BuildTopRecipesFromPreferencesUseCase;
...
public class Demo3 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path currentPath = Paths.get(System.getProperty("user.dir"));
        Path filePath = currentPath.resolve("Preferences.txt");
        List<String> lines = Files.readAllLines(filePath, StandardCharsets.UTF_8);
        if (lines.size() < 3) {
            System.err.println("Preferences.txt must have 3 lines: <query>, <mealType>, <cuisineType>");
            return;
        }
        String qValue           = lines.get(0).trim();
        String mealTypeValue    = lines.get(1).trim();
        String cuisineTypeValue = lines.get(2).trim();
        ...
        // 2) Wire dependencies and call the use case
        RecipeGateway gw = new MealDbApiGateway();
        BuildTopRecipesFromPreferencesUseCase uc = new BuildTopRecipesFromPreferencesInteractor(gw);

        List<Recipe> top = uc.execute(qValue, mealTypeValue, cuisineTypeValue);
        if (top.isEmpty()) {
            System.out.println("No meals found. Try simplifying your query.");
            return;
        }

        // 3) Write recipes.txt in your original-ish format
        List<String> outLines = new ArrayList<>();
        for (Recipe meal : top) {
            outLines.add(meal.name);
            for (String line : meal.ingredients) {
                outLines.add("  - " + line);
            }
            if (!meal.instructions.isEmpty()) {
                outLines.add("Instructions:");
                for (int i = 0; i < meal.instructions.size(); i++) {
                    outLines.add((i + 1) + ". " + meal.instructions.get(i));
                }
            }
            outLines.add("");
        }
